        ...
        try{
            ...
            if(db.rst.next()){
                if(newpassword.equals(confirmpassword)){
                    ...
                    if(rowAffected!=0){
                        response.sendRedirect("ChangeAdminPassword.jsp?msg=password changed successfull");
                    }
            }else{
                   response.sendRedirect("ChangeAdminPassword.jsp?error=new password and confirm password does not match");

                
                }
            }else{
                response.sendRedirect("ChangeAdminPassword.jsp?error=Old password is inccorect");
            }
